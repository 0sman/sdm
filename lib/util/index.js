...
const requests = xssec?.v3?.requests || xssec.requests;
...
async function fetchAccessToken(credentials, jwt) {
  const { clientCredentialsAccessToken } = getConfigurations();
  if(clientCredentialsAccessToken) {
    return getClientCredentialsToken(this.creds);
  }
...
function isRepositoryVersioned(repoInfo, repositoryId) {
  // Ensure repository is created as non-versioned, versioned repositories are not supported by the plugin.
  // Overriding this method due to issues on dms side related to changing repository type after creation.
  const repoType = "non-versioned";
