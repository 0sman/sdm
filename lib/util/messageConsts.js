...
module.exports.largeFileError = (largeFiles, maxFileSizeMb) => {
  const bulletPoints = largeFiles.map(file => `• ${file}`).join('\n');
  return `The following files could not be uploaded as they exceed the maximum allowed file size of ${maxFileSizeMb} MB:\n${bulletPoints}\n`;
}
