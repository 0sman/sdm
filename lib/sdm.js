...
module.exports = class SDMAttachmentsService extends (
  ...
  async get(attachments, keys, req) {
    ...
    /*
      ...
      ---------------------------------
      We always resolve the real attachment ID from the request URL to avoid parent/child ID collisions.
      Why not trust keys.ID? The upstream @cap-js/attachments plugin.js derives keys from the URL using a regex(attachmentIDRegex).
      When both parent and child compositions share the key name 'ID', that regex can capture the parent ID.
      Using the wrong ID means our SELECT does not find the attachment row and returns no url. Therefore we
      ignore the provided keys.ID and re-extract the child ID ourselves.
    */
    let attID = null;
    if (req?.req?.url) {
      // Prefer explicit attachments(...) segment
      const seg = req.req.url.match(/\/attachments\(ID=([0-9a-fA-F-]{36})/);
      if (seg) {
        attID = seg[1];
      } else {
        /*
          Collect every 'ID=<uuid>' token and pick the last one (child). Path structure nests parent first, child last.
          If that yields no result we fail fast
        */
        const all = req.req.url.match(/ID=([0-9a-fA-F-]{36})/g);
        if (all && all.length) attID = all[all.length - 1].replace('ID=', '');
      }
    }
    if (!attID) {
      // Do NOT fall back to keys.ID (could be wrong parent ID). Fail fast for clearer diagnostics.
      req.reject(400, 'Unable to extract attachment ID from URL; download aborted.');
      return;
    }
    let response = await getURLFromAttachments({ ID: attID }, attachments);
    const token = await fetchAccessToken(
      this.creds,
      req.user.tokenInfo.getTokenValue()
    );
    try {
      // CAP can return an array if the key predicate is not fully unique (e.g. drafts + actives) or via projections.
      const Key = Array.isArray(response) ? response.find(r => r?.url)?.url : response?.url;
      if (!Key) {
        req.reject(404, 'Attachment url not found (objectId missing).');
        return;
      }
  ...
  async draftSaveHandler(req) {
    if (req.data.messages && req.data.messages.length > 0) {
      // If there are already error/warning messages, skip attachment processing
      return;
    }
    await this.checkRepositoryType(req);
    const attachmentsTarget = cds.model.definitions[req.target.name].elements["attachments"].target;
    ...
    // If there are attachments to delete, delete them first to avoid conflicts
    await this.attachDeletionData(req);
    await this.deleteAttachmentsWithKeys(null, req);
