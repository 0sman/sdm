...
module.exports = class SDMAttachmentsService extends (
  ...
  async create(attachment_val_create, attachments, req, token, renameError){
    ...
    const maxFileSizebytes = attachments['@MaxFileSizeBytes'] || 0;
    ...
    if(largeFiles.length != 0){
      errorResponse = errorResponse + largeFileError(largeFiles, maxFileSizebytes);
    }
...
async isFileNameDuplicateInDrafts(data,req) {
  ...
  async onCreate(data, credentials, token, attachments, req, parentId, maxFileSizebytes) {
    let failedReq = [],
      Ids = [],
      success = [],
      success_ids = [];
    await Promise.all(
      data.map(async (d) => {
        ...
        if (d.content === null) {
          ...
          Ids.push(d.ID);
        } else if(maxFileSizebytes > 0 && d.content.length > maxFileSizebytes) {
